...
P2 all
```{r, fig.width=10, fig.height=2}
ggplot(aes(x=gene_annotation, y=-log10(p2), colour=group), data=Data)+geom_point()+theme_pub()+geom_hline(yintercept=-log10(1.07*10^(-9)))+xlab("Gene")+ylab("-log10(p-value)")+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=4))+ theme(legend.position = "right")+scale_color_discrete(labels=c("Cytochrome B", "DNA Polymerase", "Glucocorticoid", "RNA Polymerase","Telomerase", "Thyroid","Melanocortin"))+guides(color=guide_legend(title="Gene Group"))
